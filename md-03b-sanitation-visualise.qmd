...
3.  How many variables are in the data set? **There are 8 variables in the data set.**
...
```{r}
sanitation |> 
  count(varname_short, varname_long, percent)
```
...
```{r}
sanitation_indonesia_2000_2020_notsafelymanaged <- sanitation |> 
  filter(name == "Indonesia",
         year == 2000 | year == 2020,
         varname_short != "san_sm")
glimpse(sanitation_indonesia_2000_2020_notsafelymanaged) # line to check the new object

# creating another object including safely managed sanitation services
sanitation_indonesia_2000_2020 <- sanitation |> 
  filter(name == "Indonesia",
         year == 2000 | year == 2020)
sanitation_indonesia_2000_2020 |> 
  count(varname_short)
```
...
```{r}
sanitation_indonesia_2000_2020_notsafelymanaged |> 
  count(year)
```
...
```{r}
ggplot(sanitation_indonesia_2000_2020_notsafelymanaged,
       mapping = aes(x = residence,
                     y = percent,
                     fill = varname_long)) +
  geom_col() +
  facet_wrap(~year) + # don't forget the tilde
  geom_text(aes(label = round(percent, 1)), 
          position = position_stack(vjust = 0.5),
          size = 3,
          color = "white") 
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? **The order of the bars and legend are automatically alphabetical**
2.  What would you want to change? **We might prefer another order, for example we might need to order based on how well-managed the sanitation services are (no sanitation - basic sanitation - unimproved sanitation - limited sanitation)**
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? **Observations "safely managed sanitation services" have the percent value of NA, so they can't be plotted (error message = containing missing values)**
...
ggplot(data = sanitation_2020,
       ...
       # not putting y value so it would automatically be count()
  geom_histogram() +
  facet_grid(varname_short ~ residence, scales = "free_y") +
    # scales = "free_y" is used so the max y doesn't have to be 100, can be any value 
  scale_fill_colorblind() +
  theme(legend.position = "none") # is used to hide the legend
